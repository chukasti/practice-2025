...
app = FastAPI(middleware=[Middleware(CSRFMiddleware, csrf_protect=csrf_protect)])
...
async def verify_origin(request: Request):
    if request.method in ("POST", "PUT", "PATCH", "DELETE"):
        ...
        if not referer or not any(
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Invalid request source"
            )
...
async def check_local_network(request: Request, call_next):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Invalid client IP"
        )
...
async def login_page(request: Request, response: Response):
    ...
    response.set_cookie(
        key=csrf.cookie_name, value=csrf_token, httponly=True, samesite="strict"
    )
    return response


@app.post("/api/login")
async def login(request: Request, auth: LoginPass):
    ...
    try:
        if not auth.login or not auth.password:
            return RedirectResponse(
                "/login?error=empty_fields", status_code=status.HTTP_303_SEE_OTHER
            )
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "DELETE FROM bruteforce_protect WHERE user_id = %s", (true_user_id,)
                )
                ...
                response = RedirectResponse(
                    "/home", status_code=status.HTTP_303_SEE_OTHER
                )
                response.set_cookie(
                    key="session_id",
                    value=token,
...
async def home_page(
    request: Request, user_data: Tuple[str, str, str] = Depends(verify_token)
):
...
async def audit_page(
    ...
    if role != "auditor":
        return templates.TemplateResponse(
            "not_enough_privileges.html", {"request": request}
        )

    return templates.TemplateResponse(
        "audit.html", {"request": request, "csrf_token": get_csrf_token(request)}
    )
